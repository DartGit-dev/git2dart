...
| EC-RL-22 | Repository state was initiated outside this package's dedicated wrappers | `state` may still expose the corresponding libgit2 enum; not every state has a high-level starter method. | `reversa/sdd/state-machines.md` | 🟢 CONFIRMED for observation; 🟡 INFERRED for initiation route |
