...
| T013 | Create the release-candidate commit containing only the approved 0.5.6 release delta and its evidence artifacts, using an English Conventional Commit message; record the exact commit SHA for hosted validation. | T011, T012 | - | `git commit` | 🟢 | `[X]` |
| T014 | Trigger or observe the Build workflow for the T013 commit and capture Quality, Linux, macOS, Windows, Android, and iOS job URLs/statuses individually; do not substitute historical 1.13.0 results. | T013 | - | `.github/workflows/build.yml` | 🟢 | `[X]` |
| T015 | Complete the evidence matrix from T014: identify each unrun, unavailable, skipped, or failed target by name and leave it unproven; keep publication and live HTTPS/SSH in separate unproven rows unless directly observed. | T014 | - | `reversa/forward/005-binaries-1-14-release-0-5-6/release-evidence.md` | 🟢 | `[X]` |
